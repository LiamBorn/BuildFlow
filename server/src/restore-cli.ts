...
import { persistRestoredFile } from "./fileDurability.js";
...
if (!force) {
  // The Replit run command sets PORT=5000 for the server, not for this CLI.
  for (const port of process.env.PORT ? [Number(process.env.PORT)] : [4300, 5000]) {
