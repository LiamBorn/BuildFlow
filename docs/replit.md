...
| `OPS_ADMIN_TOKEN` | Opens the ops endpoints under `/api/ops` (take and list backups, platform counts, runtime stats), sent as the `x-ops-token` header. In production, leaving it unset keeps them closed.. Ten wrong tokens from one address locks that address out of all four routes for fifteen minutes; a correct token is never counted and a success clears earlier failures, so polling with the right token is never affected |
