...
| `ANTHROPIC_API_KEY` | Not needed on Replit: turn on **AI Integrations for Anthropic** (Replit managed) and the app receives `AI_INTEGRATIONS_ANTHROPIC_API_KEY` and `AI_INTEGRATIONS_ANTHROPIC_BASE_URL`, with the calls billed to the Replit account's credits; the startup log then says `LIVE … through Replit AI Integrations`. Set `ANTHROPIC_API_KEY` only to use an Anthropic account of your own instead — it wins when both are present. Either way, BuildFlow AI answers for real — and every answer is a paid call, so this is the one secret that turns a request into money. Both AI routes are capped for that reason: 40 questions an hour and 6 schedule imports an hour, counted per customer account, or per address for visitors on the shared demo (they share one account, so counting them per account would let the first spend everyone's). With NEITHER configured — no `ANTHROPIC_API_KEY`, no `ANTHROPIC_AUTH_TOKEN`, and AI Integrations off — the routes answer `{mode:"demo"}`, the client simulates, and nothing is billed. |
...
| `WAITLIST_ADMIN_TOKEN` | Guards `POST /api/waitlist/announce`, the launch broadcast. One accepted call emails every waitlist subscriber not yet written to, from your own domain, so this is the most consequential single request in the API and this token is all that guards it — make it long and random. Leave it unset until the day you want to send it: the route then answers 503 and cannot be called at all. Five attempts an hour per address. |
| `OPS_ADMIN_TOKEN` | Opens the ops endpoints under `/api/ops` (take and list backups, platform counts, runtime stats). Send it as the `x-ops-token` header rather than the `?token=` form these routes also accept: a query string is recorded in full by Replit's edge log, by any proxy in front of it and in browser history, and a secret in a log is a secret you have to rotate. In production, leaving it unset keeps the routes closed rather than open. Ten wrong tokens from one address locks that address out of all four for fifteen minutes; a correct token is never counted and a success clears earlier failures, so polling with the right token is never affected. |
