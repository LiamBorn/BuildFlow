...
export function ScheduleImportDialog({
  ...
  return (
    <div className="project-dialog-backdrop" role="presentation">
      <section
        className="project-dialog sim-dialog"
        role="dialog"
        aria-modal="true"
      ...
      >
              ...
              {preview.projects.map((project) => (
                <div className="sim-project" key={project.name}>
                  ...
                  {project.jobCount > project.sampleJobs.length && (
                    <p className="sim-more">
                      + {(project.jobCount - project.sampleJobs.length).toLocaleString()} more activities
                    </p>
                  )}
                </div>
              ))}
        ...
        <footer className="sim-actions">
          ...
          <div className="sim-actions-right">
            <button
              type="button"
              className={stage === "done" ? "primary-button" : "sim-ghost"}
              onClick={onClose}
              disabled={busy}
            >
            ...
            {(stage === "preview" || stage === "committing") && (
              <button type="button" className="primary-button" onClick={commit} disabled={busy || totalJobs === 0}>
                {stage === "committing" ? (
                  <>
                    <Loader2 className="spin" size={16} aria-hidden="true" /> Importing…
                  </>
                ) : (
                  `Import & see full health check`
                )}
              </button>
            )}
          </div>
...
function ConfidenceBand({ confidence }: { confidence: NonNullable<ScheduleForecastIQ["confidence"]> }) {
  ...
  return (
    <div className="sim-band">
      ...
      <div className="sim-band-track" role="img" aria-label={`Likely finish between ${confidence.p10} and ${confidence.p90}; P50 ${confidence.p50}, P80 ${confidence.p80}`}>
  ...
  return (
    <div className={`sim-forecastIQ sim-tone-${tone}`}>
      ...
      <div className="sim-forecastIQ-dates">
        ...
        <div className="sim-forecastIQ-arrow" aria-hidden="true">→</div>
        <div>
  ...
  return (
    <div className="sim-health">
      ...
      <div className="sim-health-stats">
        <div><b>{health.stats.activities.toLocaleString()}</b><span>activities</span></div>
        <div><b>{health.stats.complete.toLocaleString()}</b><span>complete</span></div>
        <div><b>{health.stats.inProgress.toLocaleString()}</b><span>in progress</span></div>
        <div><b>{health.stats.notStarted.toLocaleString()}</b><span>not started</span></div>
        <div><b>{health.stats.relationships.toLocaleString()}</b><span>links</span></div>
      </div>
