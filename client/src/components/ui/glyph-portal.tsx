...
export default function GlyphPortal({
  ...
  return (
    <section ref={sectionRef} id={uid} data-gp-root className={className} aria-label={text}
      style={{ "--gp-length": length, "--gp-characters": Array.from(text).length, ...style } as CSSProperties}>
      {/* Styles live in src/glyph-portal.css, scoped to [data-gp-root]. */}
      {/* JS reveals only measured geometry. The unmeasured poster is reserved for no-JS. */}
      <noscript><style>{`[data-gp-root] [data-gp-fallback]{display:grid}[data-gp-root] [data-gp-hint]{display:none}`}</style></noscript>
