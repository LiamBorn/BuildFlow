...
// `revealKey` lets a single-page app re-scan for freshly-mounted [data-reveal]
// nodes when the active view changes (otherwise they'd stay at opacity:0).
export function useHudMotion(rootRef: RefObject<HTMLElement | null>, revealKey?: unknown) {
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Re-scan on every revealKey change; skip nodes already revealed.
    const targets = Array.from(root.querySelectorAll("[data-reveal], [data-reveal-stagger]")).filter(
    ...
    // Safety net: if the observer hasn't fired shortly after a view swap (e.g.
    // headless/hidden tab), force the reveal so content never gets stuck hidden.
    const fallback = window.setTimeout(() => targets.forEach((el) => el.classList.add("in")), 600);
    return () => {
      observer.disconnect();
